class Knight extends MovableObject {
  ...
  IMAGES_HURT = ["img/knight/hurt/hurt 0.png", "img/knight/hurt/hurt 1.png"];
  IMAGES_DEAD = [
  ...
  constructor(delay = 0, startX = 800, moveRange = 100, id) {
    // Add the delay, startX, and moveRange parameters
    ...
    setTimeout(() => {
      // Delay the start of the knight's movement
  ...
  loadImages(images) {
    // Load the images for the knight
    images.forEach((path) => {
      // Load the images
  ...
  animate() {
    ...
    this.attackAnimationInterval = setInterval(() => {
      // Attack animation
      if (this.isAttacking) {
        // If the knight is attacking
        this.playAnimation(this.IMAGES_ATTACKING);
      }
    }, 500 / 2); // Further reduce the attack animation speed
  }
  handleMovement() {
    // Handle the knight's movement
    if (!this.dead && this.isMoving) {
      // Nutze die Property statt Methode
  ...
  playDeathAnimation() {
    if (!this.deathAnimationPlayed) {
      // Todesanimation nur einmal starten
  ...
  removeKnight() {
    if (this.world && this.world.knights) {
      // Überprüfen, ob this.world und this.world.knights definiert sind
      const knightIndex = this.world.knights.findIndex(
